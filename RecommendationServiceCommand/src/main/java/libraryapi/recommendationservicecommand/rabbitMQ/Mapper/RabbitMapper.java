package libraryapi.recommendationservicecommand.rabbitMQ.Mapper;
...
public class RabbitMapper {
    ...
    public static TempLending StringToTempLending(String input) {
        Pattern pattern = Pattern.compile(
                "TempLending\\{id=(null|\\d+),\\s*lendingCode='(.*?)',\\s*bookId=(\\d+),\\s*bookTitle='(.*?)',\\s*" +
                        "lendDate=(\\d{4}-\\d{2}-\\d{2}),\\s*limitDate=(\\d{4}-\\d{2}-\\d{2}),\\s*" +
                        "returnedDate=(null|\\d{4}-\\d{2}-\\d{2}),\\s*daysTillReturn=(\\d+),\\s*" +
                        "returned=(true|false),\\s*daysOverdue=(\\d+),\\s*fine=([\\d\\.]+),\\s*" +
                        "comment='(.*?)',\\s*readerId=(\\d+),\\s*recommended=(true|false)\\}"
        );
        ...
        if (matcher.find()) {
            ...
            String comment = matcher.group(12);
            Long readerId = Long.parseLong(matcher.group(13));
            boolean recommended = Boolean.parseBoolean(matcher.group(14));

            TempLending tempLending = new TempLending();
            tempLending.setId(id);
            tempLending.setLendingCode(lendingCode);
            tempLending.setReaderId(readerId);
            tempLending.setBookId(bookId);
            tempLending.setBookTitle(bookTitle);
            tempLending.setLendDate(lendDate);
            tempLending.setLimitDate(limitDate);
            tempLending.setReturnedDate(returnedDate);
            tempLending.setDaysTillReturn(daysTillReturn);
            tempLending.setReturned(returned);
            tempLending.setDaysOverdue(daysOverdue);
            tempLending.setFine(fine);
            tempLending.setComment(comment);
            tempLending.setRecommended(recommended);
    ...
    public static List<Reader> StringToReaderList(String input) {
        ...
        String[] readerStrings = input.split("(?<=\\}),\\s*(?=Reader\\{)");

        for (String readerString : readerStrings) {
            readerString = readerString.trim();
            if (!readerString.isEmpty()) {
                Reader reader = StringToReader(readerString);
                if (reader != null) {
                    readers.add(reader);
                }
            }
    ...
    public static Reader StringToReader(String input) {
        ...
        if (matcher.find()) {
            try {
                ...
                System.err.println("Error parsing reader string: " + e.getMessage());
                e.printStackTrace();
                return null;
            }
        }
    ...
    public static Genre StringToGenre(String input) {
        ...
        if (matcher.find()) {
            try {
                ...
                System.err.println("Error parsing genre string: " + e.getMessage());
                return null;
            }
        }
        return null;
    ...
    public static List<Genre> stringToGenreList(String input) {
        ...
        String[] genreStrings = input.split("(?<=\\}),\\s*(?=Genre\\{)");

        for (String genreString : genreStrings) {
            genreString = genreString.trim();
            if (!genreString.isEmpty()) {
                Genre genre = StringToGenre(genreString);
                if (genre != null) {
                    genres.add(genre);
                }
            }
